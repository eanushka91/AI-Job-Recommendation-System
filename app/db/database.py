...
def get_db_connection():
    ...
    if not all([DB_HOST, DB_PORT, DB_NAME, DB_USER, DB_PASSWORD]):
        logger.error("Database connection cannot be established: Configuration is missing.")
...
def init_db():
    ...
    try:
        ...
        if conn and not conn.closed: # Check if connection exists and is not already closed
            conn.close()
