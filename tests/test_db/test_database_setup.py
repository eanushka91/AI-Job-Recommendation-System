...
def db_conn_for_integration_tests():
    try:
        ...
        conn_setup = psycopg2.connect(
            host=DB_HOST, port=DB_PORT, dbname=DB_NAME, # Use TEST_* variables if configured
            user=DB_USER, password=DB_PASSWORD
        )
        yield conn_setup
        conn_setup.close()
    except (ImportError, psycopg2.OperationalError) as e:
        pytest.skip(f"Skipping integration tests: Test DB connection/config failed: {e}")
...
def test_get_db_connection_mocked(mocker):
    ...
    mock_conn_obj = mocker.MagicMock(name="mock_db_conn_success") # Mock connection object
    ...
    mock_connect.assert_called_once_with(
        host='mock_host', port='mock_port', dbname='mock_db',
...
def test_get_db_connection_failure_mocked(mocker):
    ...
    mock_connect = mocker.patch("psycopg2.connect", side_effect=psycopg2.OperationalError("Mock connection failure"))
...
def test_init_db_connection_error_mocked(mocker):
    ...
    mock_get_conn = mocker.patch("app.db.database.get_db_connection", side_effect=ConnectionError("Mock DB connection failed in init"))
    mock_create_tables = mocker.patch("app.db.database.create_tables")
