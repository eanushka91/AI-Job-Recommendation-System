...
class JobAPIService:
    ...
    def fetch_jobs(
        ...
        try:
            ...
            print(
                "Warning: JOOBLE_API_KEY not found as an attribute in app.config.settings."
            )

        if not api_key:
            print(
                "Warning: Jooble API key is not configured or not found. Cannot fetch jobs."
            )
            return []
        ...
        try:
            ...
            print(
                f"JobAPIService: Sending API request to Jooble. Params: {search_query}, Key ending: {api_key_display}"
            )
            ...
            if response.status_code == 200:
                print(
                    f"JobAPIService: Successfully received Jooble API response for page {page}, status {response.status_code}"
                )
                return JobAPIService._process_jooble_response(response.json())
            else:
                print(
                    f"JobAPIService: Jooble API error. Status: {response.status_code}, Response: {response.text}"
                )
                return []
        except requests.exceptions.Timeout:
            print(
                "JobAPIService: Error fetching jobs from Jooble API - Request timed out."
            )
            return []
        except requests.exceptions.RequestException as e:
            print(
                f"JobAPIService: Error fetching jobs from Jooble API (RequestException) - {str(e)}"
            )
            return []
        except Exception as e:
            print(
                f"JobAPIService: Unexpected error fetching jobs - {type(e).__name__}: {str(e)}"
            )
            return []
